...
const ChatInput = ({value, onChange, onSend, appendMessage, ...props}) => {
  ...
  const checkFileSize = React.useCallback((f, cb, noCheck) => {
    if (f.fileSize || f.size) {
      if (noCheck || (f.fileSize || f.size) < 3000000) cb();
      else Alert.alert('The file must be less than 3mb');
    }
  }, []);
  ...
  const sendPicOrVideo = React.useCallback(async (source, noCheckSize) => {
    checkFileSize(source, async () => {
